SELECT
    ...
    CO.ENQ_CONTACT_NAME,
    EP.PRESENTING_ISSUE,
    -- SSIS uses c.[CP] directly without coalesce
    CO.CP,
    CO.LOCATION,
    CO.HOSPITAL,
    CO.CONTACT_METHOD,
    ...
    CASE
        WHEN CO.PROGRAM_STREAM_DESC ILIKE 'CMHW%'
            THEN 'Community Mental Health & Wellbeing Services'
        WHEN CO.PROGRAM_STREAM_DESC = 'Pop-Up Mental Health Services'
            THEN 'Community Mental Health & Wellbeing Services'
    ...
    CASE
        WHEN CO.STO IS NULL
          AND (
              CO.PROGRAM_STREAM_DESC = 'Pop-Up Mental Health Services'
              OR CO.PROGRAM_STREAM_DESC ILIKE 'CMHW%'
          ) THEN 'Valid'
        WHEN CO.STO IS NOT NULL
          AND CO.PROGRAM_STREAM_DESC = 'Early Intervention Psychsocial Support'
            THEN 'Valid'
        ELSE 'Not Valid'
    END                                                   AS REPORTING,
    CO.COST,
...
WHERE CO.PROGRAM_STREAM_DESC IN (
    'Early Intervention Psychsocial Support',
    'Pop-Up Mental Health Services',
    'CMHW Tier 1/Intensive Site',
    'CMHW Tier 2/Moderate Site',
    'CMHW Tier 3/Low Site',
    'CMHW Tier 4/Bridging Support'
)
