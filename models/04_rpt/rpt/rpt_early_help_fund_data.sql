SELECT
    ...
    CO.STO,
    CASE
        WHEN CO.PROGRAM_STREAM_CODE = 'EH'
          AND CO.ORD_SUB_CAT IN (
              'Social worker', 'Dietetics', 'Occupational therapy',
              'Physiotherapy_DHS', 'Speech Pathology', 'Counselling'
          ) AND CO.HOSPITAL IN (
              'Midwifery', 'Child Health Team', 'Child and Family Services'
          ) THEN 'COP - Early Help'
        WHEN CO.PROGRAM_STREAM_CODE = 'IFSEH'
          AND CO.ORD_SUB_CAT = 'IRIS Activity Type'
          AND CO.HOSPITAL IN (
              'Midwifery', 'Child Health Team', 'Child and Family Services'
          ) THEN TRIM(CO.PROGRAM_STREAM_DESC)
        WHEN CO.PROGRAM_STREAM_CODE = 'EH'
          AND CO.ORD_SUB_CAT = 'IRIS Activity Type'
          AND CO.HOSPITAL IN (
              'Midwifery', 'Child Health Team', 'Child and Family Services'
          ) THEN 'Family Services - Early Help'
        ELSE 'not supported program stream/ordersubcat'
    END                                                   AS PROGRAM,
    CO.REPORTING_QTR,
...
WHERE CO.PROGRAM_STREAM_CODE IN ('IFSEH', 'EH')
  AND CO.HOSPITAL IN ('Midwifery', 'Child Health Team', 'Child and Family Services')
  ...
  AND CO.UR IS NOT NULL
  AND CO.EPISODE_ID IS NOT NULL