...
SELECT
    ...
    CASE
        WHEN CO.ORD_SUB_CAT_RESOLVED = 'Respite Care'
          AND CO.HOSPITAL = 'Carer Respite'
            THEN 'Support for Carers'
        ELSE 'Do not use ' || CO.PROGRAM_STREAM_DESC
    END                                                   AS PROGRAM,
    CASE
        WHEN CO.ORD_SUB_CAT_RESOLVED = 'Respite Care'
          AND CO.HOSPITAL = 'Carer Respite'
    ...
    CASE
        WHEN CO.ORD_SUB_CAT_RESOLVED = 'Respite Care'
          AND CO.HOSPITAL = 'Carer Respite'
            THEN 'DFFH Support for Carers Program'
        ELSE 'Do not use'
    END                                                   AS STREAM,
    CO.STO,
    CO.OEORDI_REF,
...
WHERE TRY_TO_NUMBER(LEFT(CO.REPORTING_QTR, 4)) >= 2020
  AND CO.PROGRAM_STREAM_CODE ILIKE 'SFC%'
  AND (
